...
def render():
    ...
    if df.empty:
        st.info("Nenhum registro encontrado. Importe planilhas para começar.")
        return
    ...
    for placa in sorted(df["placa"].dropna().unique()):
        ...
        with st.expander(f"🚛  {placa}  —  R$ {custo:,.2f}  |  {ltrs:,.0f} L  |  {kml:.2f} km/L"):
            c1, c2, c3 = st.columns(3)
            c1.metric("💰 Custo Total",  f"R$ {custo:,.2f}")
            c2.metric("🛢️ Litros",       f"{ltrs:,.0f} L")
            c3.metric("📈 Eficiência",   f"{kml:.2f} km/L", delta_color=cor_kml)

            df_v = v[["data","produto","valor","quantidade","horimetro","responsavel"]].copy()
